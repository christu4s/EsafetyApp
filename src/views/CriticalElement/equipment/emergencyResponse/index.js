...
export const EquipmentEmergencyResponse = () => {
    ...
    useEffect(() => {
        ajax.get('/equipment_emergency_response').then(res => res && setContent(res));
        ajax.get('/response_add_sce').then(res => res && setData(res.data));
    }, []);
    const showModal = () => {
    ...
    async function saveData() {
        ...
        await ajax.post('/equipment_emergency_response', values).then(res => res && setContent(res));
        setEditMode(!editMode);
    }
    function submit() {
        var { title = '', response_add = '' } = form.getFieldsValue();
        ajax.post('/response_add_sce', { title, response_add: response_add ? response_add.file : null }).then(res => {
                ...
                <Col span={16}>
                    ...
                    <Row>
                        {data.map((v, i) => <Col key={i} span={8}>
                            <Card className='custom--card' hoverable style={{ width: 200 }} cover={<img alt="example" src={v.response_add.length ? v.response_add[0].src : image} />}>
